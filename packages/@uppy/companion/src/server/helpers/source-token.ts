...
/**
 * Minted token plus its exact expiry, so callers can expose the expiry without
 * decoding the bearer token.
 */
export type MintedSourceToken = {
  token: string
  expiresAt: number
}

type MintSourceTokenOptions = {
  secret: string
  sourceId: string
...
export function mintSourceTokenWithExpiry({
  secret,
  sourceId,
  sourceVersion,
  userHash,
  ...
  ttlMs = TOKEN_TTL_MS,
}: MintSourceTokenOptions): MintedSourceToken {
...
export function mintSourceToken(options: MintSourceTokenOptions): string {
  return mintSourceTokenWithExpiry(options).token
}
