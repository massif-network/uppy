  ...
  protected async loginOAuth({
    ...
    try {
      ...
      const token = await new Promise<string>((resolve, reject) => {
        ...
        // a COOP header on that page severs our handle to it and `window.closed`
        // falsely reports `true` — which would abort a perfectly valid, in-progress
        // authentication (SmugMug's login page does this). Success arrives over the
        // WebSocket; cancellation comes via `signal` (the user dismissing the auth
        // UI). A safety timeout prevents leaking the socket and pending promise if
