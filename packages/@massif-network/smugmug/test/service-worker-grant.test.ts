...
describe('buildServiceWorkerGrant', () => {
  ...
  test('canonicalizes an HTTPS companion URL to its origin', () => {
    const grant = buildServiceWorkerGrant(
      'https://companion.example.com/some/path',
      { 'uppy-auth-token': 'tok' },
      1000,
    )
  ...
  test('does not treat inherited object properties as allowlisted headers', () => {
    expect(() =>
